...
class UpdateRequest extends FormRequest
{
    ...
    {
        return [
        'cedula'=>'string|required|max:8|unique:clientes,'.$this->route('clientes')->id.'',
        'nombre'=>'string|required|max:40,'.$this->route('clientes')->id.'',
        'apellido'=>'string|required|max:40,'.$this->route('clientes')->id.'',
        'telefono'=>'string|required|max:11|unique:clientes,'.$this->route('clientes')->id.'',
        'correo'=>'string|unique:clientes|max:100|email:rfc,dns,'.$this->route('clientes')->id.'',
        'direccion'=>'string|required|max:250,'.$this->route('clientes')->id.''
        ];
    }
